...
{
    private readonly IEnumerable<char> _controlChars =
    [
        '\u0000',
        '\u0001',
        '\u0002',
    ...
    public void TakesFalseResultOnAllControlsOneNot()
    {
        IBool equality = new IsControlCondition(
            _controlChars.Select(x => new Char(x)).Append(new Char('A'))
        );
    ...
    {
        IBool equality = new IsControlCondition(
            _controlChars.Select(x => new Char(x)).First()
        );
        Assert.True(equality.BoolValue);
    }
    ...
    public void ThrowsExceptionOnGetHashCode()
    {
        _ = Assert.Throws<NotSupportedException>(() =>
            new IsControlCondition(new Char('A')).GetHashCode()
        );
    }
    ...
    {
        _ = Assert.Throws<NotSupportedException>(() =>
            new IsControlCondition(new Char('A')).ToString()
        );
    }
}
