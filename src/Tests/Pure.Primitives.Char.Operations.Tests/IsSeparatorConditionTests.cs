...
public sealed record IsSeparatorConditionTests
{
    ...
    {
        Assert.Throws<NotSupportedException>(() => new IsSeparatorCondition(new Char('A')).GetHashCode());
    }

    [Fact]
    public void ThrowsExceptionOnToString()
    {
        Assert.Throws<NotSupportedException>(() => new IsSeparatorCondition(new Char('A')).ToString());
    }
}